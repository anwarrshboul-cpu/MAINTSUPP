...
async function applyMigrations(d1: D1DatabaseLike) {
  ...
  /* THE ASSETS SECTION. Last of the schema stages and deliberately so: it
     ...
     Independent of the Job Type stage above: that one extends
     `maintenance_requests`, `compliance_documents` and `contractors`, this one
     `units` and `unit_service_records`, so neither can see the other's tables
