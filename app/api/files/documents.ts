 ...
 * second copy of it.
 */
export async function outsideSiteScope(
  db: Db,
  orgId: string,
  siteScope: string[],
  record: {
...
/** What both upload doors answer — a missing record, as a read would say. */
export const UPLOAD_OUTSIDE_SCOPE = "The record this file is for was not found.";

/**
 * The site restriction on the UPLOAD doors — `POST /api/files` and the
 * multipart route.
 *
 * Both checked the organisation, the anchors' existence and a capability, and
 * never the member's sites: a member confined to three stores who held
 * `board.edit` could file a document against any job, site or asset in the
 * workspace, and a replacement could stand down a document they may not even
 * read. The question asked is `outsideSiteScope`'s, of:
 *
 *   - what the upload will be filed against — the supplied anchors plus, for a
 *     new version, whatever it inherits (`effectiveAnchors`); and
 *   - for a new version, the document it supersedes, because a document the
 *     member may not read is not one they may change (the write doors' rule).
 *
 * `false` at once for an unrestricted member (`siteScope` null), so they run no
 * extra query and see exactly what they saw before. The CALLER decides whose
 * restriction applies: only a capability upload is the member's own; a job
 * link or a request form is its grant's, and the member's scope says nothing
 * about it.
 */
export async function uploadOutsideSiteScope(
  db: Db,
  orgId: string,
  ...
  const asRecord = (anchors: Anchors) => ({
    ...
    requestId: anchors.requestId || null,
    contractorId: anchors.contractorId || null,
  });
  if (await outsideSiteScope(db, orgId, siteScope, asRecord(filed))) return true;
  ...
  const [predecessor] = await db
    ...
    .from(attachments)
    .where(and(eq(attachments.id, replacesId), eq(attachments.organisationId, orgId)))
    .limit(1);
  /* A predecessor that does not resolve is `planVersion`'s 404 to give. */
  if (!predecessor) return false;
  return outsideSiteScope(db, orgId, siteScope, predecessor);
}
