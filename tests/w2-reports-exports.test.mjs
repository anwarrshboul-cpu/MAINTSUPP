...
/* ── The two documents, and the fact that neither contains the other ─────── */

/*
 * THE ASSERTION THAT MATTERS HERE IS THE ONE ON THE BYTES.
 *
 * A Report export must contain no invoice section and an Invoice export no
 * report section — not merely "the model filters them", which is a property of
 * a function, but "the file that leaves this process does not have the string
 * in it", which is the property a client would notice. So the fixture is
 * rendered three times per format and the generated bytes are read back.
 *
 * The two probes are chosen to be unmistakable and unique to their part:
 * "Total payable" appears only in the invoice summary and site charges, and
 * "Full Job Log" only in §11. If a renderer ever printed a part heading over an
 * empty set, or walked `document.sections` instead of `sectionsFor`, one of
 * these appears in the file it does not belong in.
 */
test("a Report export carries no invoice section and an Invoice export no report section", () => {
  const payload = samplePayload();
  const invoiceOnly = "Total payable";
  const reportOnly = "Full Job Log";

  const readDocx = (bytes) => utf8(readStoredZip(bytes).get("word/document.xml"));
  /*
   * Every part of the workbook, joined. The sheets carry the content, the
   * workbook carries the tab names and `docProps/core.xml` carries the title —
   * and a leak into any of the three would be a leak.
   */
  const readXlsx = (bytes) => {
    const parts = readStoredZip(bytes);
    return [...parts.keys()]
      .filter((name) => name.endsWith(".xml"))
      .map((name) => utf8(parts.get(name)))
      .join("\n");
  };

  const report = {
    docx: readDocx(renderDocx(payload, "report")),
    pdf: latin1(renderPdf(payload, "report")),
    xlsx: readXlsx(renderXlsx(payload, "report")),
  };
  const invoice = {
    docx: readDocx(renderDocx(payload, "invoice")),
    pdf: latin1(renderPdf(payload, "invoice")),
    xlsx: readXlsx(renderXlsx(payload, "invoice")),
  };

  for (const [format, text] of Object.entries(report)) {
    assert.ok(
      !text.includes(invoiceOnly),
      `the ${format} Report contains "${invoiceOnly}" — an invoice figure reached a maintenance report`,
    );
    assert.ok(text.includes(reportOnly), `the ${format} Report is missing its own job log`);
    assert.ok(text.includes("MAINTSUPP Maintenance Report"), `the ${format} Report is untitled`);
  }
  for (const [format, text] of Object.entries(invoice)) {
    assert.ok(
      !text.includes(reportOnly),
      `the ${format} Invoice contains "${reportOnly}" — a client's bill carried somebody's job history`,
    );
    assert.ok(text.includes(invoiceOnly), `the ${format} Invoice is missing its own total`);
    assert.ok(text.includes("MAINTSUPP Invoice"), `the ${format} Invoice is untitled`);
  }

  // And the combined document, unchanged, still carries both.
  const combined = readDocx(renderDocx(payload));
  assert.ok(combined.includes(invoiceOnly) && combined.includes(reportOnly));
  assert.ok(combined.includes("Invoice and Maintenance Performance Report"));
});

test("the part filter is the model's, so no renderer has to remember it", () => {
  const payload = samplePayload();
  const ids = (kind, audience = "internal") =>
    sectionsFor(buildReportDocument(payload, kind), audience).map((section) => section.id);

  assert.deepEqual(ids("invoice"), ["invoice-summary", "site-charges"]);
  assert.equal(ids("report").length, 11, "sections 3 to 13");
  assert.ok(!ids("report").includes("invoice-summary"));
  assert.ok(!ids("invoice").includes("full-job-log"));
  assert.equal(ids("combined").length, 13);

  /*
   * The default is the DOCUMENT'S kind and not "everything". That is the whole
   * safety property: a renderer asks for its sections and gets the ones that
   * belong to the document it was handed, with no argument to forget. Passing
   * a part explicitly still overrides it, which is what the two on-screen tabs
   * do over a combined document.
   */
  const combined = buildReportDocument(payload, "combined");
  assert.equal(sectionsFor(combined, "internal", 1).length, 2);
  assert.equal(sectionsFor(combined, "internal", 2).length, 11);
  assert.equal(sectionsFor(combined, "internal", null).length, 13);

  // A single-part document drops the "Part 1 — " numbering: a file whose every
  // page is Part 2 reads as an extract of something the reader was not given.
  assert.deepEqual(
    buildReportDocument(payload, "report").parts.map((part) => part.title),
    ["Maintenance Performance Report"],
  );
  assert.deepEqual(
    buildReportDocument(payload, "invoice").parts.map((part) => part.title),
    ["Invoice"],
  );
  assert.equal(buildReportDocument(payload, "combined").parts.length, 2);
});
...
/*
 * A Report and an Invoice for the same client and period land in the same
 * downloads folder on the same afternoon. Before the kind was in the name they
 * collided — the second download silently becoming `…(1).docx` or overwriting
 * the first, with nothing in the folder saying which was which.
 */
test("a Report file and an Invoice file for one client and period cannot collide", () => {
  const payload = samplePayload();
  const base = {
    clientName: payload.invoice.clientName,
    periodStart: payload.period.start,
  ...
  const report = exportFilename({ ...base, kind: "report" });
  const invoice = exportFilename({ ...base, kind: "invoice" });
  assert.notEqual(report, invoice);
  assert.equal(report, "MAINTSUPP_Report_Smith-Co-UK-EU-U-nit-3_2026-03_v1.docx");
  assert.equal(invoice, "MAINTSUPP_Invoice_Smith-Co-UK-EU-U-nit-3_MS-000241.docx");

  // The same aggressive sanitiser, on every segment. See the note above on why
  // `U-nit` is the contract behaving as written.
  for (const name of [report, invoice]) {
    assert.ok(!/[\\/:*?"<>|]/.test(name));
    assert.ok(!name.startsWith("."));
  }

  // The version is the report's, and defaults to 1 until the register keeps one.
  assert.match(
    exportFilename({ ...base, kind: "report", version: 4 }),
    /_v4\.docx$/,
    "a re-issued report must not overwrite the one that was sent",
  );

  // A draft invoice has no reference, so the month stands in for one — two
  // drafts for two months are still two files.
  assert.equal(
    exportFilename({ ...base, kind: "invoice", invoiceNumber: null }),
    "MAINTSUPP_Invoice_Smith-Co-UK-EU-U-nit-3_DRAFT-2026-03.docx",
  );

  // And a caller that says nothing is named exactly as it always was.
  assert.equal(exportFilename(base), exportFilename({ ...base, kind: "combined" }));
  assert.equal(
    exportFilename(base),
    "MAINTSUPP_Smith-Co-UK-EU-U-nit-3_2026-03-01_2026-03-31_MS-000241.docx",
  );
});
