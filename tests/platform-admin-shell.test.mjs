 ...
 *
 * SIX since the enquiries inbox. `leads` was added the day `GET /api/leads` stopped
 * being a 501, which is the rule in `platform-sections.ts` being kept rather than
 * bent: a rail entry is a promise that there is something behind it. The four
 * screens with no server side are still absent, and the test below still proves it.
 */
const ENTRIES = [
